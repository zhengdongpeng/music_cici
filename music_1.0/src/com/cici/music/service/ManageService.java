...
import com.cici.music.dao.ManageDao;
...
public class ManageService {
	...
	public String login(MUser user, HttpServletRequest request) {
		...
		MUser u=manageDao.queryMUser(user);
		if(u == null){
		...
		if(user.getUsername().equals(u.getUsername())
			...
			json.put("success", "��½�ɹ�");
			request.getSession().setAttribute("manage", u);
		}else{
			json.put("error", "账号或密码不存在");
			return json.toJSONString();
		}
	...
	public String createmanage(MUser user, HttpServletRequest request) {
		JSONObject json =new JSONObject();
		json.put("stats", "error");
	if(StringUtils.isEmpty(user.getUsername())){
			json.put("error", "账号为空");
		...
		int i=manageDao.insertMUser(user);
		if(i>0){
			json.put("stats", "success");
			json.put("success", "��½�ɹ�");
		}else{
	...
	public String updatemanage(MUser user, HttpServletRequest request) {
		JSONObject json =new JSONObject();
		json.put("stats", "error");
	if(StringUtils.isEmpty(user.getUsername())){
	...
	public String queryForName(HttpServletRequest request) {
		...
		List<MUser> user = manageDao.queryMuser(map);
		json.put("list", user.toArray());
		return json.toJSONString();
	}

	public String updatemanage(HttpServletRequest request) {
		...
		map.put("jibie",jibie );
		int i=manageDao.updateMJibie(map);
		if(i>0){
			json.put("stats", "success");
			json.put("success", "成功");
		}else{
