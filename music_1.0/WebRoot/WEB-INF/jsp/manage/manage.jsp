<%@ page language="java" contentType="text/html; charset=utf-8"   import="java.util.*,com.cici.music.pojo.*"
    pageEncoding="utf-8"%>
    <% MUser manage = (MUser)session.getAttribute("manage");  %>
...
background:url(images/manage/bgk.png) ;
...
<body background="images/manage/bgt1.gif" class="htbg">
<div id="wrapper">
<div>
...
<div  class="right">
       ...
       <% if(manage.getJibie()==0){ %>
       	  ...
       	  管理员级别设定：	<select  class="jibie">
       		...
       		<button onclick="createmanage()">确认创建</button>
       		<br>
       		<br>
       		<br>
       		<p>查询账号</p>
       		...
       		<button onclick="querymanage()">查询账号</button>
       		<br>
       		...
       		<button onclick="deletemanage()">删除账号</button>
       		<br>
       		<br>
       		<br>
       		<%} %>
       		...
       		再次输入新密码<input name="password3" class="password3" type="password"/>
       			<button onclick="updatemanage()">确认修改</button>
       </div>
 </div>
</div>
</body>
		  ...
		  url:"managequery.do",
		    ...
		    success:function(data){
		    	...
		    	for(var i=0;i<list.length;i++){
		    		...
		    		"</td><td width='200px'><a onclick=\"modifymanage('"+i+","+list[i].username+"')\">权限修改</a>|<a onclick=\"deletemanage('"+list[i].username+"')\">删除</a</td>"
...
function deletemanage(username){
	debugger;
	if(!username)
